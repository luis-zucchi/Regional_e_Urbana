...
lista_decis_1_skills_todos <- list()
lista_decis_1_sofst_todos <- list()
...
for(i in 1:4){
  for(decilo in 1:11){
    ...
    lista_decis_1_skills_todos[[decilo]] <- merge(lista_decis_1[[decilo]],result_decis_todos[[i]], by.x= "codigo", by.y = "municipio")
    
    setDT(lista_decis_1_skills_todos[[decilo]])
    
    lista_decis_1_sofst_todos[[decilo]] <- lista_decis_1_skills_todos[[decilo]][, .(media_ncognitivo = mean(ncognitivo, na.rm = TRUE),
                                                                                    media_nsocial = mean(nsocial, na.rm = TRUE),
                                                                                    media_nmotor = mean(nmotor, na.rm = TRUE),
                                                                                    media_nmed = mean(nmed, na.rm = TRUE))]
    gc()
  }
  lista_final_todos[[i]] <- lista_decis_1_sofst_todos
  gc()
}
...
for (i in seq_along(caminhos_19_22)) {
  ...
  if (ano %in% c(2019:2022)) {
    ...
    dados <- fread(caminhos_19_22[i],
                   select = c("municipio", "clascnae95", "empem3112", "naturjur", "ocup2002")) %>%
      .[naturjur>2020 & as.numeric(empem3112)==1 &  (clascnae95>=50000 & clascnae95<75000 | clascnae95>= 80000 & clascnae95 < 95000 | clascnae95 >=99000)] %>%
...
lista_final <- list()
lista_decis_1_skills <- list()
lista_decis_1_sofst <- list()
...
for(i in 1:4){
  for(decilo in 1:11){
    ...
    lista_decis_1_skills[[decilo]] <- merge(lista_decis_1[[decilo]],result_decis[[i]], by.x= "codigo", by.y = "municipio")
    
    setDT(lista_decis_1_skills[[decilo]])
    
    lista_decis_1_sofst[[decilo]] <- lista_decis_1_skills[[decilo]][, .(media_ncognitivo = mean(ncognitivo, na.rm = TRUE),
                                                                        media_nsocial = mean(nsocial, na.rm = TRUE),
                                                                        media_nmotor = mean(nmotor, na.rm = TRUE),
                                                                        media_nmed = mean(nmed, na.rm = TRUE))]
  }
  lista_final[[i]] <- lista_decis_1_sofst
}
