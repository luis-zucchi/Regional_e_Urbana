...
for (categoria in 1:4) {
  ...
  for (ano in serie_anos) {
    ...
    for (arquivo in seq_along(caminhos)) {
      ...
      if (ano_arquivo == as.character(anos_interesse[ano])) {
        ...
        print(ano_str)
        print(caminhos[arquivo])
        
        dados <- fread(caminhos[arquivo],
...
diretorio <- "D:\\Pessoal\\Luis_Souza\\Agropecuaria\\rais skills\\ranking\\todos os setores\\decis\\"
